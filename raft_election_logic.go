	...
	for _, peerId := range this.peersIds {
		go func(peerId int) {
			...
			if err := this.server.SendRPCCallTo(peerId, "RaftNode.RequestVote", args, &reply); err == nil {
				...
				// }
				//-------------------------------------------------------------------------------------------/
				// Handle RequestVote's reply
				if reply.Term > this.currentTerm {
					// If we receive a reply with a higher term, then update our term and transition to follower state.
					this.currentTerm = reply.Term
					this.becomeFollower(reply.Term)
					return
				} else if reply.Term == this.currentTerm {
					// If the term matches our current term, then count the vote if it's for us and check if we have won the election.
					if reply.VoteGranted && this.state == "Candidate" {
						votesReceived++
...
func (this *RaftNode) becomeFollower(term int) {
	...
	// Cancel any running election timers.
	this.lastElectionTimerStartedTime = time.Time{}

	// Update the node state to follower.
	this.state = "Follower"

	// Update the current term and reset the voted for field.
